...
@section('content')
    <section class="news">
        <div class="container">
            <h2 class="title">@lang('main.menu.zhizni-kampusa')</h2>
            <div class="row">
                @forelse($items as $key=>$item)
                    @if ($key == 0)
                        <div class="col-lg-6 col-md-4 col-sm-6">
                            <div class="news-card">
                                <a href="{{ action('PageController@showZhizniKampusa', $item->id) }}">
                                    <div class="news-card__img"
                                        style="background-image: url({{ Upload::hasFile('news', $item->id) ? Upload::getThumbFile('news', $item->id, '555x224') : '' }});">
                                    </div>
                                </a>
                    ...
                    @else
                        <div class="col-lg-3 col-md-4 col-sm-6">
                            <div class="news-card">
                                <a href="{{ action('PageController@showZhizniKampusa', $item->id) }}">
                                    <div class="news-card__img"
                                        style="background-image: url({{ Upload::hasFile('news', $item->id) ? Upload::getThumbFile('news', $item->id, '262x224') : '' }});">
                                    </div>
                                </a>
                                <div class="news-card__info news-card__info--low-height">
                                    {{-- <p class="news-card__date">{{ Date::parse($item->created_at)->format('j F Y,  H:i') }}</p> --}}
