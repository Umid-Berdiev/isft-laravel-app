...
<div class="row">
    <div class="col-12">
        <div class="card-box table-responsive">
            <div class="row m-0">
                <div class="col-sm-6">
                    <h4 class="m-t-0 m-b-30 header-title">ACCA</h4>            
                </div>
                <div class="col-sm-6">
                    <a href="{{ route('backend.acca.getform') }}" class="btn btn-success pull-right"><i class="fa fa-plus"></i> Добавить</a>
                </div>
            </div>
...
<script>  
    ...
    var form = "{{ route('backend.acca.getform') }}";
    var crud = new Crud({
        filter: true,
        list: {
            url: "{{ route('backend.acca.data') }}",
            datatable: {            
                columns: [
                    {data: 'id', name: 'id'},
